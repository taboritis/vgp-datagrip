...
-- "e-tron, e-tron SB W tym modele S"--
...
BEGIN TRAN Transaction1
	   ...
	   values ('Audi Perfect Lease Poziom 3e dla klientów prywatnych (24m)', '<b>Opcje zakończenia</b><br>2 opcje zakończenia leasingu:<br>1) jednorazowa spłata wartości końcowej<br>2) przekazanie pojazdu do Dealera<br><br>Jest to oferta dla klientów prywatnych, przygotowana przez Volkswagen Leasing GmbH Sp. z o.o. Oddział w Polsce. Prezentowane miesięczne raty to wartość netto rat dla oferty leasingowej Audi Perfect Lease. Warunki produktu określa umowa.', 'NEW_PERFECT_LEASE_LVL_3_E_BRUTTO_24', 24, 5, 15000, 24, '0,5,10,15,20,25,30', 0, 3.15, 0, current_timestamp,0,0,'2022-01-01', @FinancialProductGroupId );
	   SET @FinancialProduc24tId = SCOPE_IDENTITY();

	   insert into financial_product([name], [description], [code], [default_finacial_time], [defualt_own_payment], default_yearly_limit, financial_time_options, own_payment_options, bank_commission, rate, deleted, created, type, required_importer, start_date, financial_product_group_id)
	   values ('Audi Perfect Lease Poziom 3e dla klientów prywatnych (36m)', '<b>Opcje zakończenia</b><br>2 opcje zakończenia leasingu:<br>1) jednorazowa spłata wartości końcowej<br>2) przekazanie pojazdu do Dealera<br><br>Jest to oferta dla klientów prywatnych, przygotowana przez Volkswagen Leasing GmbH Sp. z o.o. Oddział w Polsce. Prezentowane miesięczne raty to wartość netto rat dla oferty leasingowej Audi Perfect Lease. Warunki produktu określa umowa.', 'NEW_PERFECT_LEASE_LVL_3_E_BRUTTO_36', 36, 0, 15000, 36, '0,5,10,15,20,25,30', 0, 3.34, 0, current_timestamp,0,0,'2022-01-01', @FinancialProductGroupId );
	   SET @FinancialProduc36tId = SCOPE_IDENTITY();

	   insert into financial_product([name], [description], [code], [default_finacial_time], [defualt_own_payment], default_yearly_limit, financial_time_options, own_payment_options, bank_commission, rate, deleted, created, type, required_importer, start_date, financial_product_group_id)
	   values ('Audi Perfect Lease Poziom 3e dla klientów prywatnych (48m)', '<b>Opcje zakończenia</b><br>2 opcje zakończenia leasingu:<br>1) jednorazowa spłata wartości końcowej<br>2) przekazanie pojazdu do Dealera<br><br>Jest to oferta dla klientów prywatnych, przygotowana przez Volkswagen Leasing GmbH Sp. z o.o. Oddział w Polsce. Prezentowane miesięczne raty to wartość netto rat dla oferty leasingowej Audi Perfect Lease. Warunki produktu określa umowa.', 'NEW_PERFECT_LEASE_LVL_3_E_BRUTTO_48', 48, 0, 15000, 48, '0,5,10,15,20,25,30', 0, 3.56, 0, current_timestamp,0,0,'2022-01-01', @FinancialProductGroupId );
...
insert into financial_product_model (financial_product_group_id, model_code, model_year, created) VALUES (@FinancialProductGroupId, 'GEN', 2022, CURRENT_TIMESTAMP);
insert into financial_product_model (financial_product_group_id, model_code, model_year, created) VALUES (@FinancialProductGroupId, 'GEA', 2022, CURRENT_TIMESTAMP);
